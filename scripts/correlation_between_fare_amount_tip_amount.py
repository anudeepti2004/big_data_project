...
taxi_data_required_field = taxi_data.map(lambda entry: (float(entry[fields['tip_amount']]),float(entry[fields['fare_amount']]))).filter(lambda x: x[0] > 0 and x[1] > 0).map(lambda x: ((x[0],x[1]),1)).reduceByKey(lambda x,y : x+y)
